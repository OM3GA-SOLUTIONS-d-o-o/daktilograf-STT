...
from daktilograf_stt_ctcdecoder import Alphabet, UTF8Alphabet
...
class _SttConfig(Coqpit):
    ...
    export_model_version: str = field(
        default="0.0.1",
        metadata=dict(
            help="semantic version of the exported model. See https://semver.org/. This is fully controlled by you as author of the model and has no required connection with Daktilograf STT versions"
        ),
    )
    ...
    export_min_stt_version: str = field_val_equals_help(
        "minimum Daktilograf STT version (inclusive) the exported model is compatible with"
    )
    export_max_stt_version: str = field_val_equals_help(
        "maximum Daktilograf STT version (inclusive) the exported model is compatible with"
    )
