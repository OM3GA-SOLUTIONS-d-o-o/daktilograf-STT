...
def check_ctcdecoder_version():
    ...
    try:
        ...
        from daktilograf_stt_ctcdecoder import __version__ as decoder_version
    except ImportError as e:
        if e.msg.find("__version__") > 0:
            print(
                "Daktilograf STT version ({ds_version}) requires CTC decoder to expose __version__. "
                "Please upgrade the daktilograf_stt_ctcdecoder package to version {ds_version}".format(
    ...
    if rv != 0:
        print(
            "Daktilograf STT version ({}) and CTC decoder version ({}) do not match. "
